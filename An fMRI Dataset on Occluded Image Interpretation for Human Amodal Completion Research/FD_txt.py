...
dataset_path = 'G:\LIBAODATA\SCI DATA\derivatives\validation\code\FD\FD_data'
fmriprep_path = pjoin(dataset_path, 'derivatives', 'fmriprep')
support_path = 'G:\LIBAODATA\SCI DATA\derivatives\validation\support_files'
...
for sub_dir in sub_dirs:
    FD_tmp = []
    files = [ _ for _ in os.listdir(pjoin(fmriprep_path, sub_dir)) if 'FD.txt' in _ \
                and int(re.findall(r'run-\d+', _)[0].split('-')[-1])<=4]
    files.sort()
    for file in files:
